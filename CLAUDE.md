...
- `PairListWindow`: floating window for batch metrology template editing. Supports multiple named configurations (product templates) via `QComboBox` selector. Each config has a name and list of `FeaturePair` entries. Shows `QTableWidget` with columns: sequence #, Feature A, Feature B, Distance (mm), Lower bound (mm), Upper bound (mm). Lower/upper bound cells are user-editable. Delete Row removes selected pair. Confirm saves/updates the named config and emits `config_confirmed(list)` with all configs. New button starts a blank config. Delete Config button removes the current template. Emits `pair_row_selected(int)` when a row is clicked, triggering measurement overlay rendering on the current frame. `get_config_names()` and `get_pairs_for_config(name)` used by BatchInspectWindow
- `BatchInspectWindow`: floating window for production inspection against saved templates. `QComboBox` selects template. Read-only `QTableWidget` with columns: #, Feature A, Feature B, Distance (mm), Lower (mm), Upper (mm), Pass (✓/✗ colored green/red, "—" when no tolerance set). Inspect button triggers grab+detect+match. Uses `BatchInspectWorker(QThread)` for async processing
- `BatchInspectWorker(QThread)`: runs `detect_lines_and_arcs` + `fuzzy_match_template_pairs` on a grabbed frame, emits `done(list)` with `(distance_mm, FeaturePair)` results or `error(str)`
...
- Batch inspect flow: BatchInspect button → `_batch_inspect_pending` flag → `_on_grab_frame` interception → `_run_batch_inspect` → `BatchInspectWorker` → `_on_batch_inspect_done` → `BatchInspectWindow.show_results`
...
- Fuzzy ID matching for batch inspection: `parse_line_id` / `parse_arc_id` parse ID strings into components. `_score_line_match` scores detected lines against a template (edge pair must match, segment distance ±1, angle ±5°, length ±15%). `_score_arc_match` scores detected arcs (grid cell ±1, radius bucket ±1). `fuzzy_match_template_pairs` uses Hungarian assignment for 1-to-1 matching between template feature IDs and detected features, then computes distances for each pair
